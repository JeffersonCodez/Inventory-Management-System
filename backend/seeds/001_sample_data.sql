...
-- User accounts are NOT seeded here. Passwords need a real bcrypt hash,
-- which this SQL file can't generate — run `npm run seed:users` after
-- `npm install` instead (see scripts/seedUsers.js and the backend README).
--
-- Transaction history is ALSO seeded by that same script rather than here,
-- since every transaction references a user_id — a foreign key that can't
-- be satisfied until the users above actually exist.
